...
namespace AcTools.ServerPlugin.DynamicConditions {
    public static class OpenWeatherApiProvider {
        ...
        private static double? TryParse(string s) {
            return double.TryParse(s, NumberStyles.Any, CultureInfo.InvariantCulture, out var value) ? value : (double?)null;
        }

        [ItemNotNull]
        public static async Task<WeatherDescription> GetWeatherAsync(string apiKey, double latitude, double longitude) {
            var requestUri = string.Format(RequestWeatherUri, latitude, longitude, apiKey);
            using (var client = new WebClient()) {
                var data = await client.DownloadStringTaskAsync(requestUri);
                ...
                try {
                    ...
                    var type = OpenWeatherTypeToCommonType((OpenWeatherType)(TryParse(weatherNode.Attribute(@"number")?.Value) ?? 0));
                    var temperature = TryParse(temperatureValue) ?? 25d;

                    var windSpeed = TryParse(windNode.Descendants(@"speed").First().Attribute("value")?.Value) ?? 0d;
                    var windDirection = TryParse(windNode.Descendants(@"direction").First().Attribute("value")?.Value) ?? 0d;

                    var humidity = TryParse(doc.Descendants(@"humidity").FirstOrDefault()?.Attribute(@"value")?.Value) ?? 70d;
                    var pressure = TryParse(doc.Descendants(@"pressure").FirstOrDefault()?.Attribute(@"value")?.Value) ?? 1000d;
                    return new WeatherDescription(type, temperature, windSpeed, windDirection, humidity, pressure);
                } catch {
